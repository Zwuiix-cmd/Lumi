...
import static cn.nukkit.entity.Entity.DATA_TARGET_EID;
...
public class BlazeShootExecutor implements EntityControl, IBehaviorExecutor {
    ...
    public boolean execute(EntityIntelligent entity) {
        ...
        if (tick2 == 0 && tick1 > coolDownTick) {
            ...
            if (tick2 > fireTick) {
                for(int i = 0; i < 3; i++) {
                    entity.getServer().getScheduler().scheduleDelayedTask(InternalPlugin.INSTANCE, () -> shootFireball(entity), i*6);
                }
                entity.getServer().getScheduler().scheduleDelayedTask(InternalPlugin.INSTANCE, () -> stopOnFire(entity), 20);
                tick2 = 0;
    ...
    protected void shootFireball(EntityLiving entity) {
        ...
        Entity projectile = Entity.createEntity(EntitySmallFireBall.NETWORK_ID, entity.level.getChunk(entity.getChunkX(), entity.getChunkZ()), nbt);
        ...
        ProjectileLaunchEvent projectev = new ProjectileLaunchEvent((EntityProjectile) projectile);
        Server.getInstance().getPluginManager().callEvent(projectev);
        if (projectev.isCancelled()) {
            projectile.kill();
        } else {
            projectile.spawnToAll();
            entity.level.addLevelEvent(entity, LevelEventPacket.EVENT_SOUND_BLAZE_SHOOT);
        }
    }

    private void startOnFire(Entity entity) {
        entity.setDataProperty(new LongEntityData(DATA_TARGET_EID, this.target.getId()));
        entity.setDataFlag(Entity.DATA_FLAGS, Entity.DATA_FLAG_CHARGED, true);
    }

    private void stopOnFire(Entity entity) {
        entity.setDataProperty(new LongEntityData(DATA_TARGET_EID, 0L));
        entity.setDataFlag(Entity.DATA_FLAGS, Entity.DATA_FLAG_CHARGED, false);
    }
}