        ...
        if (entity.distanceSquared(target) <= 2.5 && attackTick > coolDown) {
            ...
            if (item.applyEnchantments()) {
                for (Enchantment enchantment : enchantments) {
                    itemDamage += enchantment.getDamageBonus(target, null);
                }
            }
            ...
            EntityDamageByEntityEvent ev = new EntityDamageByEntityEvent(entity, target, EntityDamageEvent.DamageCause.ENTITY_ATTACK, damage, knockBack, null);
