...
public class RuntimeItemMapping {
    ...
    synchronized boolean registerCustomItem(CustomItem customItem) {
        ...
        if (!Server.getInstance().getSettings().getGeneral().isEnableExperimentMode()) {
            return false;
        }
    ...
    synchronized void deleteCustomItem(CustomItem customItem) {
        String namespaceId = customItem.getDefinition().identifier();
        if (!Server.getInstance().getSettings().getGeneral().isEnableExperimentMode() && !this.customItems.contains(namespaceId)) {
            return;
        }
    ...
    public void generatePalette() {
        ...
        for (RuntimeEntry entry : this.itemPaletteEntries) {
            if (entry.isCustomItem() && (!Server.getInstance().getSettings().getGeneral().isEnableExperimentMode() || protocolId < ProtocolInfo.v1_16_100)) {
                break;
            }
            size++;
        }
        ...
        for (RuntimeEntry entry : this.itemPaletteEntries) {
            if (entry.isCustomItem()) {
                if (Server.getInstance().getSettings().getGeneral().isEnableExperimentMode() && protocolId >= ProtocolInfo.v1_16_100) {
