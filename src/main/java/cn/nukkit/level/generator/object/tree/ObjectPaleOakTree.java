...
public class ObjectPaleOakTree extends TreeGenerator {
    ...
    private static final Block PALE_OAK_LOG = Block.get(BlockID.PALE_OAK_LOG);
    private static final Block PALE_OAK_LEAVES = Block.get(BlockID.PALE_OAK_LEAVES);
        ...
        if (k >= 1 && k + i + 1 < 256) {
            ...
            if (!this.placeTreeOfHeight(level, position, i)) {
                ...
                for (int j2 = 0; j2 < i; ++j2) {
                    ...
                    if (material == Block.AIR || material == Block.PALE_OAK_LEAVES) {
    ...
    private void placeLogAt(ChunkManager worldIn, Vector3 pos) {
        if (this.canGrowInto(worldIn.getBlockIdAt(pos.getFloorX(), pos.getFloorY(), pos.getFloorZ()))) {
            this.setBlockAndNotifyAdequately(worldIn, pos, PALE_OAK_LOG);
        }
    }
    ...
    private void placeLeafAt(ChunkManager worldIn, int x, int y, int z) {
        ...
        if (material == Block.AIR) {
            this.setBlockAndNotifyAdequately(worldIn, blockpos, PALE_OAK_LEAVES);
        }
    }
}
